...
class YoloV3:
    ...
    def run(self, image):
        ...
        boxes_, classes, scores = self.sess.run(
            [self.BOXES_NAME, self.CLASSES_NAME, self.SCORES_NAME],
            feed_dict={self.INPUT_NAME: images})
        ...
        return boxes_, classes.astype(np.int64), scores
...
if __name__ == '__main__':
    ...
    model = YoloV3('D:/Smoke/YOLOv3_TensorFlow-master/yolov3_frozen_graph_batch.pb')
    ...
    # img = np.asarray(img, np.float32)
    # images = img[np.newaxis, :] / 255.
    boxes_, labels_, scores_ = model.run(files)
