...
def wangshifu_inference(img_data):
    ...
    img_base = Image.fromarray(cv2.cvtColor(img_data, cv2.COLOR_BGR2RGB))
    ...
    image_rize_decode = base64.b64decode(image_rize_base64)
    image_rize_cv2 = cv2.imdecode(np.frombuffer(image_rize_decode, np.uint8), -1)
    qingxidu = getImageVar(image_rize_cv2)
    if qingxidu < 25:
        contect = 'NO CLEAR'
        logo = '0'
        ...
        print('===============首先看图片中有没有完整的人脸===============')
        im2 = img_base
        im2_base64 = image_to_base64(im2)
        img_base_decode_1 = base64.b64decode(im2_base64)
        img_raw = cv2.imdecode(np.frombuffer(img_base_decode_1, np.uint8), -1)

        # img_raw = cv2.imdecode(np.frombuffer(img_base_decode, np.uint8), -1)
        # 给出一个人脸检测器：
        detector = dlib.get_frontal_face_detector()
        # 用它来检测人脸，返回图中人脸的个数和坐标位置：
        faces = detector(img_raw, 1)  # print(len(faces))
        if len(faces) == 0:
...
if __name__ == '__main__':
    t1 = time.time()
    img = cv2.imread('/root/liweiwei/wangshifu2/1.jpg')
    # print(img)

    # img_base64_encode = base64.b64encode(img)
    # img_base64_decode = base64.b64decode(img)
    # img_base = cv2.imdecode(np.frombuffer(img_base_decode, np.uint8), cv2.IMREAD_COLOR)
    result_wangshifu = wangshifu_inference(img)  # 自己测试版本不用jsonify,用json dumps
